...
export async function listProblems(req, res) {
  ...
  try {
    ...
    // MULTI-TAG FILTERING (OR logic: problem has AT LEAST ONE selected tag)
    if (tagsParam) {
      // 1. SANITIZE INPUT: Convert to array of clean, lowercase tag strings
      let rawTags = Array.isArray(tagsParam)
        ? tagsParam.join(", ")
        : String(tagsParam);
      ...
      if (requestedTags.length > 0) {
        filtered = filtered.filter((p) => {
          ...
          // 2. SANITIZE DATABASE TAGS: Same cleaning process
          const cleanDbTags = p.Tags.replace(/[\[\]' "]/g, "");
          const problemTagsArray = cleanDbTags
            .split(/[,|;]/)
            .map((t) => t.trim().toLowerCase())
            .filter(Boolean);

          // 3. EXACT MATCH (AND logic): Problem matches only if it has ALL requested tags
          return requestedTags.every((requestedTag) =>
            problemTagsArray.includes(requestedTag),
          );
        });
      }
    }
